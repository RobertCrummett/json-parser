...
static void json_read_entire_file_to_cstr(const char *path, char **data, size_t *size) {
	...
	if (data == NULL || size == NULL) {
		fprintf(stderr, "The data and/or size cannot be NULL\n");
		return;
	}
	...
	*size = 0;
	...
	while (1) {
		...
		n = fread(*data + used, 1, JSON_READ_ENTIRE_FILE_CHUNK, file);
		if (ferror(file)) {
			if (*data != NULL)
				free(*data);
			fclose(file);
...
static json_array_t *json_create_array(void) {
	json_array_t *array = (json_array_t *)malloc(sizeof *array);
	if (array == NULL) {
...
static void json_array_append(json_array_t *array, json_value_t *value) {
	...
	json_array_t *new = NULL;

	/* Do not do anything if NULL is passed as either array or the value */
	...
	new = (json_array_t *)malloc(sizeof *new);
	if (new == NULL) {
...
static void json_print_array(json_array_t *array) {
	...
	/* Do not print anything when the array is NULL */
	if (array == NULL)
		return;
...
static uint64_t json_hash_string(const char *string) {
	...
	size_t i = 0;
	uint64_t hash = 1099511628211UL;

	/* For each octet of data to be hashed... */
	for (i = 0; i < strlen(string); i++)
		hash = (hash ^ (uint8_t)string[i]) * 14695981039346656037UL; /* The magic number is the FNV-1a 64-bit offset basis */
	return hash;
}
...
static json_object_t* json_create_object(size_t capacity) {
	...
	size_t i = 0;
	json_keyval_t *content = NULL;

	/* Allocate the new object */
	json_object_t* object = (json_object_t *)malloc(sizeof *object);
	if (object == NULL) {
	...
	content = (json_keyval_t *)calloc(capacity, sizeof *content);
	if (content == NULL) {
...
static void json_free_object(json_object_t **object) {
	...
	for (i = 0; i < (*object)->capacity; i++) {
		/* Check if the value exists - if it does,
		   ...
		   value. */
		if ((*object)->content[i].key != NULL) {
			free((*object)->content[i].key);
...
static int json_object_expand(json_object_t *object) {
	size_t j = 0;
	json_keyval_t *new_content = NULL;
	...
	size_t new_capacity = object->capacity < 2 ? JSON_OBJECT_DEFAULT_SIZE : 2 * object->capacity;
	...
	new_content = (json_keyval_t *)calloc(new_capacity, sizeof *new_content);
	if (new_content == NULL) {
	...
	for (j = 0; j < object->capacity; j++) {
		...
		if (bucket.key != NULL) {
			...
			uint64_t hash = json_hash_string(bucket.key);
...
static int json_object_set(json_object_t *object, char *key, json_value_t *value) {
	...
	hash = json_hash_string(key);
	index = hash % object->capacity;
	...
	while (1) {
		if (object->content[index].key == NULL) {
			...
			if (strcmp(object->content[index].key, key) == 0)
...
uint64_t hash = json_hash_string(key);
...
static void json_free_all_the_tokens(json_token_t **token) {
	json_token_t *now = NULL;
	json_token_t *later = NULL;
...
static json_token_t *json_lexer(const char *json_data, size_t json_size) {
	...
	while (*json_data != '\0' && json_size-- != 0) {
		...
		json_token_t *token = (json_token_t *)malloc(sizeof *token);
		if (token == NULL) {
...
static json_value_t *json_parser(json_token_t **tokens) {
	...
	element = (json_value_t *)malloc(sizeof *element);
	if (element == NULL) {
	...
	switch ((*tokens)->identity) {
		case JSON_TOKEN_CURLY_OPEN:
			...
			element->value.object = json_create_object(JSON_OBJECT_DEFAULT_SIZE);
			...
			*tokens = (*tokens)->next;
			...
			while (1) {
				...
				*tokens = (*tokens)->next;
				...
				if (json_object_set(element->value.object, key, value)) {
		...
		case JSON_TOKEN_SQUARE_OPEN:
			...
			element->value.array = json_create_array();
			...
			*tokens = (*tokens)->next;
			...
			while (1) {
				...
				json_array_append(element->value.array, value);
		...
		case JSON_TOKEN_NUMBER:
			...
			element->value.number = strtod((*tokens)->cstr, NULL);
			break;

		case JSON_TOKEN_BOOLEAN:
			element->identity = JSON_BOOLEAN;
			element->value.boolean = strcmp((*tokens)->cstr, "true") == 0 ? 1 : 0;
			break;
...
void json_free(json_value_t **json) {
	...
	switch ((*json)->identity) {
		case JSON_OBJECT:
			json_free_object(&(*json)->value.object);
			break;
		case JSON_ARRAY:
			json_free_array(&(*json)->value.array);
			break;
		case JSON_STRING:
...
void json_print(json_value_t *json) {
	...
	switch (json->identity) {
		case JSON_OBJECT:
			json_print_object(json->value.object);
			break;
		case JSON_ARRAY:
			json_print_array(json->value.array);
			break;
		case JSON_STRING:
...
json_value_t *json_load(const char *path) {
	char *data = NULL;
	size_t size = 0;
	json_token_t *tokens = NULL, *head = NULL;
